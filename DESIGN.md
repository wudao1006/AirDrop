...
当前 Windows/Linux 优先实现已经落地：持久化 Ed25519 身份、mDNS、TLS 1.3/QUIC、六位验证码配对、签名同步组清单与删除声明、成员方向策略、文本/URL/HTML/RTF/图片/文件剪贴板槽位，以及“只更新远端槽位、用户明确选择后才写入本机剪贴板”的完整主链路。

文件剪贴板会创建有界静态快照，拒绝符号链接和路径逃逸，使用权限受限缓存目录，通过逐文件 SHA-256 清单和 QUIC 二进制流传输。接收端持久化同一传输 ID 的部分文件并返回逐文件偏移，流中断会自动重试，断线重连后从已落盘偏移续传；完整哈希验证通过后才原子提交为可取用槽位。

仍需在发布前完成：两台真实 Windows/Linux 设备的组邀请、文本/URL/富文本/图片/文件互传矩阵；普通文件投送中心；安全的应用私有格式能力注册表；Windows 安装包代码签名。设计文档后续章节描述的是完整 v1 目标，未在本节列为已完成的能力不能据此视为当前实现。

### 1.2 2026-07-14 安全与一致性加固记录（v0.1.7）

本轮针对实现审查中发现的认证重放、配对提交、槽位正文一致性和预览隐私问题完成以下修正：

- QUIC 服务端不再使用 `with_no_client_auth`。配对和可信 ALPN 都强制客户端提交长期设备证书并完成 TLS CertificateVerify，只接受 Ed25519 握手签名和单个有界证书链。
- 可信连接以 TLS 握手实际取得的对端证书为认证输入：出站连接继续固定目标证书，入站连接在应用层授权前要求实际客户端证书与直接配对记录或有效组成员记录完全一致。Hello 签名只保留为兼容期内的附加检查，不再承担客户端持钥证明职责，因此记录过的旧 Hello 不能脱离对应 TLS 私钥重放冒用。
- 首次配对的 Responder 会校验 PairInit 声明的证书与 TLS 客户端证书完全一致；双方都完成 PairConfirm 后只发送 PairComplete，必须实际收到对端 PairComplete 才把 pending trust 原子提升为 trusted。等待完成阶段断线会删除 pending trust，不提前清除撤销记录。
- 每个远端设备增加进程内单调 sequence 高水位。正文缓存、槽位 Snapshot 和导入操作都绑定同一 sequence；旧序号不能先覆盖正文再被 UI 丢弃，ImportIntent 在最终确认时也会再次检查正文 sequence，槽位在确认期间更新时要求用户重新选择。
- `previewText`、`previewImages` 和 `previewFileNames` 现在由 Rust Snapshot 生成端执行。关闭策略会立即重算远端槽位预览并清理当前剪贴板的对应展示字段，敏感正文不再依赖某个 React 组件自行隐藏。
- 悬浮球“使用槽位”只创建 ImportIntent 并打开主窗口确认，不再在单次悬浮球点击后自动调用 ConfirmImport，恢复“选择”和“最终写入确认”两个独立动作。

该变更把应用版本提升到 v0.1.7。v0.1.6 及更早客户端不会提交 TLS 客户端证书，混合版本不再属于受支持组合；特定拨号方向即使因旧服务端未请求客户端证书而偶然完成握手，也不具备本轮双向认证保证。已有设备双方升级后仍使用原长期证书和配对记录，不需要重新比较验证码。
