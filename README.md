...
- 使用 TLS 1.3/QUIC 传输文本、HTML/RTF、图片和文件剪贴板槽位；图片使用有界单向流，文件使用带偏移协商与最终确认的可续传双向流，正文不嵌入控制 JSON。
...
桌面本地状态、设备发现、配对、签名同步组以及文本、HTML/RTF、图片、可续传文件剪贴板交换已经使用真实 Rust 服务。当前实现仍属于 v0.1：普通文件投送中心、真实 Windows/Linux 多机矩阵和 Windows 正式代码签名仍未完成。浏览器预览继续使用隔离的演示客户端，仅用于 UI 开发，不具备局域网能力。
