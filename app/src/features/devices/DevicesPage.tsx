  ...
  return <div className="page">
    ...
    <section className="page-section"><h2>已授权设备</h2><div className="card list-card">
      {snapshot.trustedDevices.length === 0 ? <EmptyState icon="devices" title="尚未配对设备" description="完成双方验证码确认后，可信设备会保存在本机。" /> : snapshot.trustedDevices.map((device) => <div className="list-row" key={device.deviceId}>
        ...
        <div className="list-row-main"><strong>{device.deviceName}</strong><span>{device.localAlias ? `对方名称：${device.advertisedName} · ` : ""}{device.platform} · 身份已固定 · {new Date(device.pairedAt).toLocaleDateString()}</span>{editingAliasId === device.deviceId && <form className="device-alias-editor" onSubmit={(event) => { event.preventDefault(); void saveAlias(device.deviceId, aliasDraft.trim() || null); }}><input className="text-field" value={aliasDraft} maxLength={48} autoFocus aria-label={`为 ${device.advertisedName} 设置本地备注名`} placeholder={device.advertisedName} onChange={(event) => setAliasDraft(event.target.value)} /><button type="submit" className="button compact primary" disabled={savingAliasId === device.deviceId}>{savingAliasId === device.deviceId ? "保存中" : "保存"}</button><button type="button" className="button compact" onClick={() => setEditingAliasId(null)}>取消</button></form>}</div>
