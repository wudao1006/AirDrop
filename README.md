...
- 使用 TLS 1.3/QUIC 传输可信设备的文本和图片剪贴板槽位；图片正文使用有界单向二进制流，不嵌入控制 JSON。
...
桌面本地状态、设备发现、配对和直接配对设备间的文本/图片槽位交换已经使用真实 Rust 服务。当前实现仍属于 v0.1：同步组授权、HTML/RTF、文件剪贴板、文件传输和完整的协议范围恢复机制尚未完成。浏览器预览继续使用隔离的演示客户端，仅用于 UI 开发，不具备局域网能力。
