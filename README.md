...
- `ready`、`metadata_only`、`stale`、`blocked` 等状态与按需获取交互；单击“使用”本身就是明确确认。
- 只有用户单击“使用”或完成跨应用拖放后才会写入/插入内容；载入或更新远端槽位不会写入本机剪贴板。
...
- 远端复制只更新独立设备槽位；用户单击“使用”后才写入本机系统剪贴板，并抑制导入后的反馈回环。
- QUIC 可信连接启用应用层保活和更宽裕的空闲超时，避免设备静置后要等下一次复制才发现断线并重连。
- 设备页按可信连接展示 RTT、上下行速率、丢包、连接时长和重连信息；传输中心显示图片/文件实时进度及最近同步结果。新版本设备会对文本、富文本和图片返回应用层接收确认，旧版本设备则明确显示为“已发送但未确认”；传输历史仅持久化元数据，每台设备保留最近 10 条、全局最多 200 条，不记录剪贴板正文。
