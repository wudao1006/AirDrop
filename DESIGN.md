...
当前 Windows/Linux 优先实现已经落地：持久化 Ed25519 身份、mDNS、TLS 1.3/QUIC、六位验证码配对、签名同步组清单与删除声明、成员方向策略、文本/HTML/RTF/图片/文件剪贴板槽位，以及“只更新远端槽位、用户明确选择后才写入本机剪贴板”的完整主链路。

文件剪贴板会创建有界静态快照，拒绝符号链接和路径逃逸，使用权限受限缓存目录，通过逐文件 SHA-256 清单和 QUIC 二进制流传输；失败流自动清理，连接仍可用时重试一次，断线重连后重放最新槽位。目前尚未实现协议级 Range 续传，重连恢复会从该文件快照开头重新发送。

仍需在发布前完成：两台真实 Windows/Linux 设备的组邀请、文本/富文本/图片/文件互传矩阵；普通文件投送中心与协议级范围续传；Windows 安装包代码签名。设计文档后续章节描述的是完整 v1 目标，未在本节列为已完成的能力不能据此视为当前实现。
