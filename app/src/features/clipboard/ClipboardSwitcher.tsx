...
export function ClipboardSwitcher({ snapshot, client, onError, compact = false }: { snapshot: UiSnapshot; client: DesktopClient; onError: (message: string) => void; compact?: boolean }) {
  ...
  const execute = (action: Promise<unknown>) => { void action.catch((reason: unknown) => onError(errorMessage(reason))); };
  return <div className={`switcher ${compact ? "summary" : ""}`}>
    ...
    {filtered.length > 0 ? <div className="slot-list">
      {filtered.map((slot) => <DeviceSlotCard key={slot.id} slot={slot} selected={selectedId === slot.id} onSelect={() => setSelectedId(slot.id)} operation={snapshot.imports.find((item) => item.slotId === slot.id)} interactionDisabled={snapshot.platform === "android" && snapshot.activity !== "foreground_live"} onUse={() => execute(client.useSlot(slot.id, slot.revision))} onConfirm={(id) => execute(client.confirmImport(id))} onCancel={(id) => execute(client.cancelImport(id))} />)}
