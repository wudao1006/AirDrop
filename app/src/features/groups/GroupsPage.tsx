...
export function GroupsPage({ snapshot, client, onError }: {
  ...
  return <div className="page">
    ...
    {creating && <section className="card group-create-panel">
      ...
      <div><strong className="field-title">允许的内容</strong><div className="group-policy-list">{([['allowText', '纯文本'], ['allowImages', '图片'], ['allowHtml', 'HTML / RTF'], ['allowFiles', '文件剪贴板']] as const).map(([key, label]) => <label key={key}><input type="checkbox" checked={policy[key]} onChange={(event) => setPolicy((current) => ({ ...current, [key]: event.target.checked }))} />{label}</label>)}</div></div></div>
